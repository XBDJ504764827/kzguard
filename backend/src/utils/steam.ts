...
const STEAM_PENDING_TEXT = '待后端识别';
...
export const resolveSteamIdentifiers = (input: string) => {
  ...
  if (!trimmedInput) {
    throw new HttpError(400, '请输入玩家 Steam 标识');
  }
  ...
  if (profileUrlMatch) {
    ...
    return { ...fromAccountId(accountId), resolved: true };
  }
  ...
  if (steamId3Match) {
    return { ...fromAccountId(BigInt(steamId3Match[1])), resolved: true };
  }
